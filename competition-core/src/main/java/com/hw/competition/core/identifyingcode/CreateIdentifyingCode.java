package com.hw.competition.core.identifyingcode;
...
public class CreateIdentifyingCode {
    ...
    public String create( HttpServletResponse response) throws IOException {
        ...
        drawRandomLine(g);
        //6.写在图片上随机数
        String random = drawRandomNum((Graphics2D) g,IdentifyingCodeConstant.NUMBER_ENGLISH);//根据客户端传递的createTypeFlag标识生成验证码图片
    ...
    private String drawRandomNum(Graphics2D g,String... createTypeFlag) {
        ...
        if (createTypeFlag.length > 0 && null != createTypeFlag[0]) {
            if (createTypeFlag[0].equals(IdentifyingCodeConstant.CHINA)) {
                // 截取汉字
                return createRandomChar(g, baseChineseChar);
            }else if (createTypeFlag[0].equals(IdentifyingCodeConstant.NUMBER_ENGLISH)) {
                // 截取数字和字母的组合
                return createRandomChar(g, baseNumLetter);
            }else if (createTypeFlag[0].equals(IdentifyingCodeConstant.NUMBER)) {
                // 截取数字
                return createRandomChar(g, baseNum);
            }else if (createTypeFlag[0].equals(IdentifyingCodeConstant.ENGLISH)) {
                // 截取字母
